...
interface CandleSpec { o: number; c: number; h: number; l: number; bull: boolean }

// Candle 座標系統：0~100（100 為最高價，0 為最低價），對應 SVG 內的相對高度。
function Candle({ o, c, h, l, bull, label }: CandleSpec & { label: string }) {
  const H = 100;
    ...
    <div className="flex flex-col items-center gap-2">
      <div className="bg-slate-50 rounded-xl px-3.5 py-2.5">
        <svg width={44} height={H} viewBox={`0 0 44 ${H}`}>
          <line x1={22} x2={22} y1={scale(h)} y2={scale(l)} stroke={color} strokeWidth={1.8} />
          <rect x={14} y={bodyTop} width={16} height={bodyH} fill={color} rx={1.5} />
        </svg>
      </div>
...
function CandleRow({ items }: { items: (CandleSpec & { label: string })[] }) {
  return (
...
// 縮小版多根K線示意圖，用在條列項目旁邊
function MiniCandles({ items }: { items: CandleSpec[] }) {
  const H = 52;
  const scale = (v: number) => H - (v / 100) * (H - 10) - 5;
  return (
    <div className="flex items-end gap-1 bg-slate-50 rounded-xl px-2.5 py-2 shrink-0" style={{ minWidth: items.length * 16 + 20 }}>
      {items.map((s, i) => {
        const color = s.bull ? "#e11d48" : "#16a34a";
        const bodyTop = scale(Math.max(s.o, s.c));
        const bodyBot = scale(Math.min(s.o, s.c));
        const bodyH = Math.max(bodyBot - bodyTop, 2);
        return (
          <svg key={i} width={16} height={H}>
            <line x1={8} x2={8} y1={scale(s.h)} y2={scale(s.l)} stroke={color} strokeWidth={1.4} />
            <rect x={4} y={bodyTop} width={8} height={bodyH} fill={color} rx={1} />
          </svg>
        );
      })}
    </div>
...
function TrendLine({ points, marks }: { points: [number, number][]; marks?: number[] }) {
  ...
  return (
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid meet">
      ...
      <path d={d} fill="none" stroke="#6366f1" strokeWidth={2.4} strokeLinejoin="round" strokeLinecap="round" />
    </svg>
  );
}

// 小型走勢圖示意圖，用在整理型態條列旁邊
function MiniTrend({ points }: { points: [number, number][] }) {
  const w = 96, h = 52, pad = 6;
  const maxX = points[points.length - 1][0];
  const X = (v: number) => pad + (v / maxX) * (w - pad * 2);
  const Y = (v: number) => pad + ((100 - v) / 100) * (h - pad * 2);
  const d = points.map((p, i) => `${i === 0 ? "M" : "L"}${X(p[0])},${Y(p[1])}`).join(" ");
  return (
    <div className="bg-slate-50 rounded-xl px-2 py-2 shrink-0">
...
function SectionCard({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h3 className="text-base font-semibold text-slate-900 flex items-center gap-2 mb-1.5">
        <span className="w-1 h-[18px] bg-indigo-500 rounded-full inline-block" />
        {title}
      </h3>
      {subtitle && <p className="text-sm text-slate-500 mb-3.5 leading-relaxed">{subtitle}</p>}
        ...
        <div key={i} className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4 px-5 py-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[180px_1fr] gap-1 sm:gap-4 flex-1 min-w-0">
            <div className="text-base font-semibold text-slate-800 flex items-center gap-2">
              {it.name}
              {it.tag && <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${tagCls[it.tag.kind]}`}>{it.tag.label}</span>}
            </div>
            <div className="text-sm text-slate-500 leading-relaxed">{it.body}</div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-slate-100 rounded-2xl overflow-hidden border border-slate-100">
      {items.map((it, i) => (
        <div key={i} className="bg-white p-4">
          <div className="text-base font-semibold text-slate-800 mb-1">{it.title}</div>
          <div className="text-sm text-slate-500 leading-relaxed">{it.body}</div>
        </div>
      ))}
    </div>
...
function Formula({ children }: { children: React.ReactNode }) {
  return <div className="bg-slate-50 rounded-xl px-4 py-3.5 text-base font-mono text-slate-700 overflow-x-auto mb-2">{children}</div>;
}

function Frac({ top, bot }: { top: string; bot: string }) {
  return (
    <span className="inline-flex flex-col items-center align-middle mx-1.5 text-sm leading-tight">
...
function PlainList({ items }: { items: React.ReactNode[] }) {
  return (
    <ul className="space-y-0">
      {items.map((it, i) => (
        <li key={i} className="text-sm text-slate-500 leading-relaxed py-2.5 border-b border-slate-50 last:border-0">{it}</li>
      ))}
    </ul>
  );
}
  ...
  return (
    <div className="max-w-4xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {TOC.map((t, i) => (
            <a key={t.id} href={`#${t.id}`}
              className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
      ...
      <div id="ch1" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.01</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">K線基礎</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">一根K線記錄開盤、收盤、最高、最低四個價格。看懂單根K線的形狀，是所有技術分析的起點。</p>
        ...
        <SectionCard title="K線的構成"
          ...
          <CandleRow items={[
            ...
            { o: 70, c: 38, h: 88, l: 35, bull: false, label: "上影黑線" },
            { o: 55, c: 72, h: 75, l: 15, bull: true, label: "下影陽線" },
            { o: 60, c: 35, h: 63, l: 12, bull: false, label: "下影黑線" },
            { o: 65, c: 78, h: 80, l: 20, bull: true, label: "陽線鐵鎚" },
            { o: 70, c: 60, h: 72, l: 15, bull: false, label: "陰線鐵鎚" },
            { o: 50, c: 52, h: 82, l: 20, bull: true, label: "十字線" },
            { o: 88, c: 90, h: 91, l: 15, bull: true, label: "T字線" },
            { o: 15, c: 13, h: 88, l: 12, bull: false, label: "墓碑線" },
      ...
      <div id="ch2" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.02</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">K線組合型態</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">把連續兩根、三根、甚至五根以上的K線放在一起看，能讀出比單根K線更完整的多空攻防過程。</p>
      ...
      <div id="ch3" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.03</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">量價關係</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">價格的漲跌需要成交量支持才走得遠；量與價脫節（背離），往往是趨勢即將反轉的前兆。</p>

        <SectionCard title="價量八法循環" subtitle="一個完整的多空循環，量與價依序經過以下八個階段：">
          <div className="bg-slate-50 rounded-xl p-4 text-sm text-slate-600 leading-loose text-center font-mono">
      ...
      <div id="ch4" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.04</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">型態學（等幅測量法）</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">股價在一段時間內走出的整體輪廓，往往比單一根K線更能預告接下來的方向與空間。</p>

        <SectionCard title="反轉型態">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div className="bg-slate-50 rounded-xl p-3">
              <TrendLine points={[[0, 30], [1, 58], [2, 45], [3, 88], [4, 50], [5, 62], [6, 48], [7, 20]]} marks={[45]} />
              <p className="text-sm text-slate-500 text-center mt-1">頭肩頂（跌破頸線構成反轉）</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <TrendLine points={[[0, 70], [1, 42], [2, 55], [3, 12], [4, 50], [5, 38], [6, 52], [7, 80]]} marks={[48]} />
              <p className="text-sm text-slate-500 text-center mt-1">頭肩底（突破頸線構成反轉）</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <TrendLine points={[[0, 25], [1, 82], [2, 48], [3, 78], [4, 15]]} marks={[46]} />
              <p className="text-sm text-slate-500 text-center mt-1">M頭（跌破頸線為賣點）</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <TrendLine points={[[0, 85], [1, 20], [2, 55], [3, 24], [4, 90]]} marks={[50]} />
              <p className="text-sm text-slate-500 text-center mt-1">W底（突破頸線完成型態）</p>
            </div>
          </div>
          <TermList items={[
      ...
      <div id="ch5" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.05</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">波浪理論</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">完整的一段趨勢，由五個推動波與三個修正波構成——「漲五跌三」，而且波中還有波。</p>

        <SectionCard title="波浪全貌">
          <div className="bg-slate-50 rounded-xl p-3">
            <TrendLine points={[[0, 20], [1, 42], [2, 30], [3, 72], [4, 52], [5, 90], [6, 64], [7, 40]]} />
          </div>
          <p className="text-sm text-slate-500 mt-2">上升五波：初升段(1)→修正(2)→主升段(3)→修正(4)→末升段(5)；回檔三波：反彈(A)→修正(B)→回跌(C)。</p>
        </SectionCard>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <span className="w-7 h-7 rounded-full bg-indigo-50 text-indigo-600 text-sm font-bold font-mono flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
      ...
      <div id="ch6" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.06</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">移動平均線與葛蘭碧八大法則</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">均線像一條會把股價拉回身邊的磁線，股價與均線的相對位置，是最經典的進出場判斷依據。</p>
            ...
            <table className="w-full text-sm border-collapse min-w-[560px]">
              <thead>
                <tr className="text-slate-400 bg-slate-50">
                  ...
                  <th className="text-left font-semibold px-3 py-2.5">法則</th>
                  <th className="text-left font-semibold px-3 py-2.5">操作要訣</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {[
                  ...
                  <tr key={r.n}>
                    <td className={`px-3 py-3 font-mono font-bold ${r.buy ? "text-red-500" : "text-emerald-600"}`}>{r.n}</td>
                    <td className="px-3 py-3 text-slate-700">{r.rule}</td>
                    <td className="px-3 py-3 text-slate-500">
                      {r.note}
                      <span className={`ml-2 text-xs font-semibold px-1.5 py-0.5 rounded ${r.buy ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-600"}`}>
                        {r.buy ? "買點" : "賣點"}
                      </span>
                    </td>
                  </tr>
        ...
        <SectionCard title="乖離率（BIAS）原理">
          ...
          <p className="text-sm text-slate-500 mb-3 leading-relaxed">均線像磁鐵，股價無論漲跌只要偏離太遠，都會被吸引拉回；正乖離過大代表獲利了結壓力大，負乖離過大代表停損殺出壓力大，兩者最終都會朝均線（零軸）修正。</p>
          <Grid2 items={[
        ...
        <SectionCard title="大盤 BIAS 參考歷史值（月線）">
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse min-w-[380px]">
              <thead>
                <tr className="text-slate-400 bg-slate-50">
                  <th className="text-left font-semibold px-3 py-2.5">均線天期</th>
                  <th className="text-left font-semibold px-3 py-2.5">賣出參考</th>
                  <th className="text-left font-semibold px-3 py-2.5">買進參考</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                <tr><td className="px-3 py-3 text-slate-700">10日乖離</td><td className="px-3 py-3 font-mono text-red-500">+5%</td><td className="px-3 py-3 font-mono text-emerald-600">−4.5%</td></tr>
                <tr><td className="px-3 py-3 text-slate-700">20日乖離</td><td className="px-3 py-3 font-mono text-red-500">+8%</td><td className="px-3 py-3 font-mono text-emerald-600">−7%</td></tr>
                <tr><td className="px-3 py-3 text-slate-700">60日乖離</td><td className="px-3 py-3 font-mono text-red-500">+14%</td><td className="px-3 py-3 font-mono text-emerald-600">−11%</td></tr>
              </tbody>
            </table>
          </div>
          <p className="text-sm text-slate-500 mt-2">個股可依熱門股（5～7成）、冷門股（約3成）的幅度自行調整。</p>
        </SectionCard>
      </div>

      {/* ===== CH7 技術指標 ===== */}
      <div id="ch7" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.07</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">技術指標</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">均量線、MACD、KD、RSI——四個最常被拿來輔助判斷買賣點的指標。</p>
      ...
      <div id="ch8" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.08</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">基本面選股</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">技術面回答「何時進出」，基本面回答「該選誰」——景氣位階與財報數字，決定一檔股票值不值得留在觀察名單。</p>
        ...
        <SectionCard title="景氣循環四階段">
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse min-w-[620px]">
              <thead>
                <tr className="text-slate-400 bg-slate-50">
                  <th className="text-left font-semibold px-3 py-2.5">項目</th>
                  <th className="text-left font-semibold px-3 py-2.5">繁榮</th>
                  <th className="text-left font-semibold px-3 py-2.5">衰退</th>
                  <th className="text-left font-semibold px-3 py-2.5">蕭條</th>
                  <th className="text-left font-semibold px-3 py-2.5">復甦</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 text-slate-500">
                <tr><td className="px-3 py-3 text-slate-700">股價循環</td><td className="px-3 py-3">上漲→下跌（多頭）</td><td className="px-3 py-3">下跌（空頭）</td><td className="px-3 py-3">下跌（空頭）</td><td className="px-3 py-3">下跌→上漲（多頭）</td></tr>
                <tr><td className="px-3 py-3 text-slate-700">金融政策</td><td className="px-3 py-3">緊張（通膨壓力）</td><td className="px-3 py-3">中立</td><td className="px-3 py-3">擴張（緊縮壓力）</td><td className="px-3 py-3">中立</td></tr>
                <tr><td className="px-3 py-3 text-slate-700">利率循環</td><td className="px-3 py-3">上升</td><td className="px-3 py-3">下跌壓力</td><td className="px-3 py-3">下跌</td><td className="px-3 py-3">上升壓力</td></tr>
                <tr><td className="px-3 py-3 text-slate-700">經濟成長率</td><td className="px-3 py-3">很高</td><td className="px-3 py-3">低</td><td className="px-3 py-3">低或負成長</td><td className="px-3 py-3">高</td></tr>
                <tr><td className="px-3 py-3 text-slate-700">失業率</td><td className="px-3 py-3">接近充分就業</td><td className="px-3 py-3">失業增加</td><td className="px-3 py-3">大量失業</td><td className="px-3 py-3">失業減少</td></tr>
              </tbody>
            </table>
          </div>
          <p className="text-sm text-slate-500 mt-2">谷底 → 復甦 → 成長 → 繁榮 → 衰退——挑選處於復甦及成長階段的產業最為有利。</p>
        </SectionCard>
      ...
      <div id="ch9" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-6 scroll-mt-20">
        <div className="text-sm font-mono text-indigo-500 mb-1">CH.09</div>
        <h2 className="text-2xl font-bold text-slate-900 mb-1.5">綜合運用——判斷六步驟</h2>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">把前面八章串成一套實際下單前會走過的完整流程。</p>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-start gap-3">
              <span className="w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 text-sm font-bold font-mono flex items-center justify-center shrink-0">{i + 1}</span>
              <div>
                <div className="text-base font-semibold text-slate-800 mb-0.5">{s.title}</div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3.5 mt-5">
          <div className="text-xs font-semibold text-amber-600 uppercase tracking-wider mb-1">補充・波段操作五階段</div>
          <div className="text-base text-slate-700 font-medium">判斷 → 選股 → 買入 → 持有 → 賣出</div>
        </div>
      </div>

      <p className="text-sm text-slate-400 text-center py-4">內容僅供技術分析教學參考，不構成個股買賣建議</p>
    </div>
  );
}
