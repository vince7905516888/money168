...
// 把「股票投資」頁面算出來的目前持股（股票代碼/名稱/股數/均價）同步進投資策略表，
// 已存在同代碼的列只更新這幾個欄位，券商/方案/未來目標價等其他手動欄位維持不變；
// 找不到對應列的持股才新增一筆
export async function POST() {
  ...
  const holdings = computeHoldings(investments).filter((h) => h.code && h.code !== "—");

  const existingEntries = await prisma.investmentStrategyEntry.findMany({
    where: { userId: session.user.id },
  });
  const byCode = new Map(
    existingEntries.filter((e) => e.stockCode).map((e) => [e.stockCode!.trim(), e])
  );
  let nextOrder = existingEntries.reduce((max, e) => Math.max(max, e.order), -1) + 1;

  const results = await Promise.all(
    holdings.map((h) => {
      const existing = byCode.get(h.code.trim());
      if (existing) {
